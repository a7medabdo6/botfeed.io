...
const PlanCard = ({ plan, onDelete, isLoading, isHighlighted }: PlanCardProps) => {
  ...
  const getCapabilitiesList = () => {
    return booleanFeatures.filter((feature) => !!plan.features[feature.id as keyof PlanFeatures]).map((feature) => t(`plan.features.${feature.id}`));
  };
    ...
    <>
      ...
      >
        {plan.is_featured && (
          <div className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 z-10">
            <span className="flex items-center gap-1 px-3 sm:px-4 py-1 sm:py-1.5 bg-(--text-green-primary) text-white text-xs sm:text-sm font-semibold rounded-full shadow-lg whitespace-nowrap">
              ...
              <span className="hidden xs:inline sm:inline">{t("plan.most_popular")}</span>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">{plan.description || t("plan.perfect_plan")}</p>
        ...
        <div className="mb-6">
          <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">{t("plan.usage_limits")}</h4>
        ...
        <div className="mb-6">
          <h4 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">{t("plan.capabilities")}</h4>
          <ul className="grid grid-cols-2 gap-x-4 gap-y-3">

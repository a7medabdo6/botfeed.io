    ...
    <div className="bg-white dark:bg-(--card-color) p-4 sm:p-6 md:p-8 rounded-lg border border-slate-200 dark:border-(--card-border-color) shadow-sm space-y-6 sm:space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
        <div className="space-y-3 flex flex-col">
          ...
          <Input placeholder="e.g. welcome_message" value={templateName || ""} onChange={(e) => setTemplateName(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))} className="h-12 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:bg-white dark:focus:bg-page-body transition-all" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
        <div className="space-y-3 flex flex-col">
          ...
          <p className="text-[11px] text-slate-400">Industry sector this template belongs to.</p>
        </div>
        ...
        <div className="space-y-3 flex flex-col">
          ...
          <Select value={templateCategory || ""} onValueChange={setTemplateCategory} disabled={!sector}>
            <SelectTrigger className="h-12 py-5.5 border-(--input-border-color) dark:border-none rounded-lg bg-(--input-color) dark:bg-page-body transition-all">
              <SelectValue placeholder={sector ? "Select category" : "Select sector first"} />
            </SelectTrigger>
            <SelectContent className="rounded-lg border-(--input-border-color) dark:border-(--card-border-color) dark:bg-page-body">
          ...
          <p className="text-[11px] text-slate-400">Specific use-case within the selected sector.</p>
        </div>
      </div>

      {/* WhatsApp Template Category */}
      <div className="space-y-4 flex flex-col">
        <Label className="text-sm font-bold text-slate-700 dark:text-slate-300">WhatsApp Category</Label>
        <div
        ...
        >
          {CATEGORIES.map((cat) => (
            <button key={cat.value} type="button" onClick={() => setCategory(cat.value)} className={`flex items-center gap-3 p-4 rounded-lg border transition-all group ${category === cat.value ? "border-primary bg-sky-50/50 text-primary dark:bg-sky-500/10" : "border-slate-100 bg-slate-50/30 dark:bg-(--table-hover) dark:border-(--table-hover) text-slate-500 hover:border-primary dark:hover:border-(--card-border-color)"}`}>
              <div className={`p-2 rounded-lg transition-colors ${category === cat.value ? "bg-sky-100 dark:bg-(--dark-sidebar)" : "bg-white dark:bg-(--dark-body) dark:text-amber-50 group-hover:bg-sky-50 dark:group-hover:bg-(--card-color)"}`}>{cat.icon}</div>
        ...
        <div className="pt-6 border-t border-slate-100 dark:border-(--card-border-color) space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2 sm:gap-3">
            {MARKETING_TYPES.map((type) => {
              ...
              return (
                <button key={type.value} type="button" onClick={() => setMarketingType(type.value)} className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all text-left group ${isActive ? "border-primary bg-sky-50/60 dark:bg-sky-500/10" : "border-slate-100 dark:border-(--card-border-color) bg-slate-50/30 dark:bg-(--table-hover) hover:border-primary/30 dark:hover:border-(--card-border-color)"}`}>
                  <div className={`p-2 rounded-lg shrink-0 transition-colors ${isActive ? "bg-sky-100 dark:bg-sky-500/20 text-primary" : "bg-white dark:bg-(--dark-body) text-slate-400 group-hover:bg-sky-50 dark:group-hover:bg-(--card-color)"}`}>{type.icon}</div>
          ...
          {marketingType === "limited_time_offer" && (
            <div className="space-y-3 flex flex-col animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <CharacterCountWrapper current={offerText?.length || 0} max={60}>
                <Input placeholder="e.g. Expires in 24 hours" value={offerText || ""} onChange={(e) => setOfferText(e.target.value.slice(0, 60))} className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:bg-white dark:focus:bg-page-body transition-all" />

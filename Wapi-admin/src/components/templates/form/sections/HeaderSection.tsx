...
export const HeaderSection = ({ templateType, setTemplateType, headerText, setHeaderText, templateTypes, setHeaderFile, headerFile }: HeaderSectionProps) => {
  ...
  return (
    <div className="bg-white dark:bg-(--card-color) p-4 sm:p-6 md:p-8 rounded-lg border border-slate-200 dark:border-(--card-border-color) shadow-sm space-y-6 sm:space-y-8">
      ...
      <div
        className="grid grid-cols-1 lg:grid-cols-2 gap-8 [@media(min-width:767)_and_(max-width:1430px)]:grid-cols-1
"
      >
        <div className="space-y-4 flex flex-col">
          ...
          <CharacterCountWrapper current={headerText?.length || 0} max={60}>
            <Input
              ...
              className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:bg-white dark:focus:bg-page-body transition-all font-medium"
            />
            ...
            {templateTypes
              ...
              .map((type) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-1.5 p-2 sm:p-3 rounded-lg border transition-all font-bold text-[9px] sm:text-[10px] uppercase tracking-wider ${templateType === type.value ? "border-primary bg-sky-50/50 text-primary dark:bg-(--table-hover)" : "border-slate-150 dark:border-(--table-hover) bg-slate-50/20 dark:bg-(--table-hover) text-slate-400 dark:text-gray-500 hover:border-primary dark:hover:border-(--card-border-color)"}`}
                >
            ...
            <div className="flex items-center justify-between p-4 bg-sky-50/50 dark:bg-sky-500/5 border border-sky-100 dark:border-sky-500/20 rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-lg bg-sky-100 dark:bg-sky-500/20 flex items-center justify-center shadow-sm">
                  <div className="text-primary dark:text-sky-400">{templateTypes.find((t) => t.value === templateType)?.icon}</div>
                </div>
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate max-w-50">{headerFile.name}</span>
                  <span className="text-[10px] text-primary dark:text-sky-400 font-bold uppercase tracking-wider">{(headerFile.size / (1024 * 1024)).toFixed(2)} MB • Ready</span>
                </div>
              </div>
              <button
                type="button"

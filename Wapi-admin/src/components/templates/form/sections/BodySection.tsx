          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {variables_example.map((variable, index) => (
              <div key={variable.key} className="p-4 rounded-lg border border-(--input-border-color) dark:border-none bg-slate-50/50 dark:bg-page-body space-y-2">
                ...
                <Input placeholder={`Example for ${variable.key}`} value={variable.example} onChange={(e) => updateVariable(index, e.target.value)} className="h-11 border-(--input-border-color) dark:border-(--card-border-color) rounded-lg bg-white dark:bg-(--dark-body) p-3 text-sm focus:border-sky-500/50 transition-all" />
              </div>
            ))}
          </div>

    ...
    <div className="bg-white dark:bg-(--card-color) p-4 sm:p-6 md:p-8 rounded-lg border border-slate-200 dark:border-(--card-border-color) shadow-sm space-y-6 sm:space-y-8">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-3">
        {interactiveActions
          ...
          .map((action) => (
            <button key={action.value} type="button" onClick={() => setInteractiveType(action.value)} className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg border transition-all text-[10px] sm:text-xs font-bold uppercase tracking-wider ${interactiveType === action.value ? "border-primary bg-sky-50/50 text-sky-700 dark:bg-(--card-color) dark:text-primary shadow-sm" : "border-slate-50 dark:border-(--card-border-color) bg-slate-50/20 dark:bg-(--dark-sidebar) dark:hover:border-(--card-border-color) text-slate-400 hover:border-slate-200"}`}>
              {action.label}
            </button>
          ))}
      </div>
      ...
      <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div className="space-y-2 flex flex-col">
                ...
                <CharacterCountWrapper current={btn.text?.length || 0} max={60}>
                  <Input
                    ...
                    className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-sky-500/50 transition-all font-medium"
                  />
              ...
              {(btn.type === "url" || btn.type === "website") && (
                <div className="space-y-2 flex flex-col">
                  ...
                  <Input placeholder="https://example.com" value={btn.url || btn.website_url || ""} onChange={(e) => updateButton(btn.id, { url: e.target.value, website_url: e.target.value })} className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-sky-500/50 transition-all font-medium" />
                </div>
              )}
              {btn.type === "phone_call" && (
                <div className="space-y-2 flex flex-col">
                  <Label className="text-xs font-bold text-slate-600 dark:text-gray-400">Phone Number</Label>
                  <Input type="number" placeholder="+1 234 567 890" value={btn.phone_number || ""} onChange={(e) => updateButton(btn.id, { phone_number: e.target.value })} className="h-11 border-(--input-border-color) p-3 dark:border-(--card-border-color) rounded-lg bg-(--input-color) dark:bg-page-body focus:border-sky-500/50 transition-all font-medium" />
                </div>
              )}
              {/* {btn.type === "copy_code" && (
                <div className="space-y-2 flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {!isRestricted && (
              <button type="button" onClick={() => addButton("phone_call")} className="flex-1 h-11 sm:h-12 flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 dark:hover:border-(--card-border-color) dark:text-amber-50 dark:border-(--card-border-color) text-slate-500 font-bold hover:border-sky-500/50 hover:bg-sky-50/30 dark:hover:bg-(--table-hover) transition-all text-xs sm:text-sm p-3">
                <Smartphone size={16} /> Add Phone Button ({buttons?.filter((b: any) => b.type === "phone_call").length || 0}/1)
              </button>
            )}
            {isRestricted && (
              <button type="button" onClick={() => addButton("copy_code")} className="flex-1 h-11 sm:h-12 flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 dark:hover:border-(--card-border-color) dark:text-amber-50 dark:border-(--card-border-color) text-slate-500 font-bold hover:border-sky-500/50 hover:bg-sky-50/30 dark:hover:bg-(--table-hover) transition-all text-xs sm:text-sm">
                <Copy size={16} /> Add Copy Code ({buttons?.filter((b: any) => b.type === "copy_code").length || 0}/1)
              </button>
            )}
          </div>
        )}
        {interactiveType === "quick_reply" && !isRestricted && (
          <button type="button" onClick={() => addButton("quick_reply")} className="w-full h-12 flex items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-200 dark:hover:border-(--card-border-color) dark:text-amber-50 dark:border-(--card-border-color) text-slate-500 font-bold hover:border-sky-500/50 hover:bg-sky-50/30 dark:hover:bg-(--table-hover) transition-all text-sm">
            <Plus size={18} /> Add Quick Reply ({buttons?.filter((b: any) => b.type === "quick_reply").length || 0}/10)
          </button>
        )}
        {interactiveType === "all" && !isRestricted && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            <button type="button" onClick={() => addButton("quick_reply")} className="flex flex-col items-center gap-2 p-3 sm:p-4 rounded-lg dark:bg-(--table-hover) dark:hover:border-(--card-border-color) border border-slate-100 dark:border-(--card-border-color) bg-slate-50/30 dark:text-amber-50 hover:border-sky-200 transition-all font-bold text-xs text-slate-600">
              <FileText size={18} className="text-slate-400" /> Quick Reply ({buttons?.filter((b: any) => b.type === "quick_reply").length || 0}/10)
            </button>
            <button type="button" onClick={() => addButton("website")} className="flex flex-col items-center gap-2 p-3 sm:p-4 rounded-lg dark:bg-(--table-hover) dark:hover:border-(--card-border-color) border border-slate-100 dark:border-(--card-border-color) bg-slate-50/30 dark:text-amber-50 hover:border-sky-200 transition-all font-bold text-xs text-slate-600">
              <Link size={18} className="text-slate-400" /> URL ({buttons?.filter((b: any) => b.type === "url").length || 0}/2)
            </button>
            <button type="button" onClick={() => addButton("phone_call")} className="flex flex-col items-center gap-2 p-3 sm:p-4 rounded-lg dark:bg-(--table-hover) dark:hover:border-(--card-border-color) border border-slate-100 dark:border-(--card-border-color) bg-slate-50/30 dark:text-amber-50 hover:border-sky-200 transition-all font-bold text-xs text-slate-600">
              <Smartphone size={18} className="text-slate-400" /> Phone ({buttons?.filter((b: any) => b.type === "phone_call").length || 0}/1)
            </button>
          </div>
        )}
      </div>

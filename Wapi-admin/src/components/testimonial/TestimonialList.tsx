  ...
  const renderStars = (rating: number) => {
    return (
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg key={star} className={`w-4 h-4 ${star <= rating ? "text-sky-500 fill-(--warning-color)" : "text-gray-300 fill-gray-300"}`} viewBox="0 0 20 20">
  ...
  const renderActions = (testimonial: Testimonial) => (
    <div className="flex items-center gap-2">
      <Can permission="update.testimonials">
        <Button variant="ghost" size="icon" onClick={() => handleEditClick(testimonial)} className="w-10 h-10 text-slate-400 hover:text-(--text-green-primary) hover:bg-sky-50 rounded-lg dark:hover:bg-primary/20 transition-all" disabled={isLoading} title="Edit">

                ...
                for (const dateGroup of draft.messages) {
                  for (const group of dateGroup.messageGroups) {
                    ...
                    if (target) {
                      ...
                      const isOutgoing = target.direction === "outbound";
                      const ourId = isOutgoing ? target.sender.id : target.recipient.id;
                      target.reactions.forEach((r) => {
                        r.users = r.users?.filter((u) => u.id !== "current-user" && u.id !== ourId) || [];
                      });

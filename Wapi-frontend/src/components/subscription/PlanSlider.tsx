...
const PlanSlider: React.FC<PlanSliderProps> = ({ plans, activePlanId, mode = "none", onSubscribe, onBack }) => {
  ...
  return (
    <div className="w-full min-h-150 select-none overflow-visible flex justify-center items-center pt-12.5 relative">
                    ...
                    <CardContent className="flex-1 flex flex-col px-4 pb-8">
                      ...
                      <div className={cn("my-6 py-6 px-4 mt-0 rounded-lg flex flex-col items-center justify-center text-center transition-colors shadow-xs border dark:bg-(--page-body-bg) dark:border-none", isActive ? cn(theme.bg, theme.border) : "bg-slate-50 dark:bg-(--page-body-bg) border-slate-100")}>
                        <div className="flex items-baseline gap-1 flex-wrap">
                          <CurrencyValue
                            amount={plan?.price}
                            fromCode={plan?.currency?.code}
                            className={cn("text-3xl font-bold tracking-tighter", isActive ? "text-slate-900 dark:text-white" : "text-slate-300")}
                            fallbackSymbol={plan.currency.symbol}
                          />
                          <span className="text-slate-400 text-xs font-semibold italic ml-auto capitalize">/{plan.billing_cycle === "monthly" ? t("subscription.per_month").replace("/", "") : plan.billing_cycle === "yearly" ? t("subscription.per_year").replace("/", "") : plan.billing_cycle}</span>
                        </div>

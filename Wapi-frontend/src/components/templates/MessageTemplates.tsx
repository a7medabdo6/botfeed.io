...
const MessageTemplates = ({ wabaId, statusFilter: statusFilterProp, onToggleSidebar }: MessageTemplatesProps) => {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-8 h-screen flex flex-col min-w-0 overflow-hidden">
      <CommonHeader title="Manage Templates" description="Manage and design your WhatsApp message templates" featureKey="template_bots_used" onRefresh={handleRefresh} onSync={handleSync} onSyncStatus={handleSyncStatus} syncStatusPermission="create.template" isLoading={isLoading} isSyncingStatus={isSyncingStatus} onAddClick={() => router.push(`/templates/${wabaId}/create`)} addLabel="Create New Template" addPermission="create.template" onSearch={setSearchQuery} searchTerm={searchQuery} searchPlaceholder="Search templates..." rightContent={WabaDisplay} onToggleSidebar={onToggleSidebar}>

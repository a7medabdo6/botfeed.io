...
const flattenMenuData = (isAgent: boolean): SearchResult[] => {
  ...
  const items = isAgent ? MENUITEMS.filter((item) => item.label === "sidebar.chat" || item.label === "sidebar.tasks").map((item) => (item.label === "sidebar.tasks" ? { ...item, path: "/tasks", section: undefined } : item)) : MENUITEMS.filter((item) => item.path !== "/tasks");

    ...
    <div className="bg-white dark:bg-(--card-color) border border-gray-100 dark:border-(--card-border-color) rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full">
      <div className="p-5 border-b border-gray-50 dark:border-(--card-border-color) flex items-start justify-between">
        ...
        <Can permission="update.ecommerce_webhooks">
          <Switch checked={webhook.is_active} onCheckedChange={() => id && onToggle(id)} disabled={isLoading} className="data-[state=checked]:bg-sky-500" />
        </Can>
      </div>

      <div className="p-5 space-y-4 flex-1">
        <div className="flex flex-wrap gap-2">
          <Badge variant="secondary" className={cn("border-none font-bold text-[10px] px-2 py-0.5", webhook.is_active ? "bg-sky-50 text-primary dark:bg-transparent" : "bg-slate-100 text-slate-500 dark:bg-(--page-body-bg)")}>
            {webhook.is_active ? "Active" : "Inactive"}
          </Badge>
          <Badge variant="secondary" className={cn("border-none font-bold text-[10px] px-2 py-0.5", !isConfigured ? "bg-rose-50 text-red-500 dark:bg-red-500/10" : "bg-sky-50 text-primary dark:bg-(--page-body-bg)")}>
            {status}
          </Badge>
        ...
        {!hasFirstPayload ? (
          ...
          <div className="bg-sky-50/30 dark:bg-sky-500/5 p-3 rounded-lg border border-sky-100/50 dark:border-sky-500/10 flex items-center gap-2">
      ...
      <div className="p-4 bg-gray-50/50 dark:bg-(--card-color) border-t border-gray-50 dark:border-(--card-border-color) flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Can permission="view.ecommerce_webhooks">
            <Button variant="ghost" size="icon" onClick={() => onViewPayload(webhook)} className="h-9 w-9 rounded-lg text-primary hover:bg-sky-50 dark:hover:bg-sky-500/10" title="Show Payload">
              <Zap size={18} />
            </Button>
          </Can>
          <Can permission="update.ecommerce_webhooks">

  ...
  return (
    <div className="bottom-0 left-0 rounded-lg relative right-0 z-50 flex-wrap [@media(max-width:352px)]:justify-center gap-2 sm:gap-0 bg-white/80 dark:bg-(--card-color)/80 backdrop-blur-xl border-t border-slate-200/60 dark:border-white/5 py-4 px-6 md:px-12 flex items-center justify-between shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
      <div className="flex items-center gap-4">
        {step > 1 && (
          <Button variant="ghost" onClick={handleBack} className="rounded-lg px-6 h-12 font-black text-slate-400 hover:text-sky-600 hover:bg-sky-50 dark:hover:bg-sky-500/10 transition-all flex items-center gap-2">
      ...
      <div className="flex items-center gap-4">
        {step === 1 ? (
          <Button onClick={handleNext} disabled={!canNext} className="bg-primary hover:bg-sky-700 text-white rounded-lg px-10 h-14 font-black transition-all hover:translate-x-1 group flex items-center gap-2">
            Next Mapping <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
          </Button>
        ) : (
          <Button onClick={handleSave} disabled={isMapping || !canSave} className="bg-primary hover:bg-sky-700 text-white rounded-lg px-12 h-11 font-black transition-all hover:scale-[1.02] flex items-center gap-3">

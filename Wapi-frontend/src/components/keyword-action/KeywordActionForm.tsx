...
import { ArrowLeft, ArrowRight, Check, Eye, FileText, GitBranch, Image, Info, LayoutTemplate, ListOrdered, Loader2, Mailbox, Plus, Search, ShoppingBag, Sparkles, Sticker, Type, Video, X } from "lucide-react";
...
import { KeywordActionCreatePayload, KeywordActionFormProps, MatchingMethod, ReplyTypeConfig } from "@/src/types/keywordAction";
...
const REPLY_TYPES: ReplyTypeConfig[] = [
  ...
  { value: "flow", label: "Form Flow", icon: <GitBranch size={18} />, color: "text-orange-500", source: "reply_material", materialType: "flow" },
];
...
const KeywordActionForm: React.FC<KeywordActionFormProps> = ({ editId }) => {
  ...
  const workspaceWabaId = selectedWorkspace?.waba_id || "";
  ...
  const actionWabaRaw = editData?.data?.waba_id as string | { _id: string } | undefined;
  const actionWabaId =
    typeof actionWabaRaw === "string"
      ? actionWabaRaw
      : actionWabaRaw && typeof actionWabaRaw === "object" && "_id" in actionWabaRaw
        ? String(actionWabaRaw._id)
        : "";

  const wabaIdForMaterials = useMemo(() => {
    if (editId && actionWabaId) return actionWabaId;
    return workspaceWabaId;
  }, [editId, actionWabaId, workspaceWabaId]);

  const skip = !wabaIdForMaterials;
  const { data: materialsData, isLoading: loadingMaterials } = useGetReplyMaterialsQuery({ waba_id: wabaIdForMaterials }, { skip });
  const { data: templatesData, isLoading: loadingTemplates } = useGetTemplatesQuery({ waba_id: wabaIdForMaterials, status: "approved" }, { skip });
  const { data: catalogsData, isLoading: loadingCatalogs } = useGetLinkedCatalogsQuery({ waba_id: wabaIdForMaterials }, { skip });
  const { data: chatbotsData, isLoading: loadingChatbots } = useGetChatbotsQuery({ waba_id: wabaIdForMaterials }, { skip });
  const { data: sequencesData, isLoading: loadingSequences } = useGetSequencesQuery({ waba_id: wabaIdForMaterials }, { skip });
                ...
                {isLoadingMaterials ? (
                  ...
                  <div className="py-14 text-center space-y-3 px-2">
                    ...
                    {activeTypeConfig.source === "chatbot" && (
                      <div className="text-xs text-slate-500 max-w-md mx-auto space-y-2 leading-relaxed">
                        <p>
                          Create an AI chatbot under{" "}
                          <Link href="/reply_materials" className="text-primary font-semibold underline underline-offset-2">
                            Reply materials → Chatbot
                          </Link>{" "}
                          for the <span className="font-semibold text-slate-600 dark:text-slate-300">same WhatsApp number</span> this keyword action belongs to.
                        </p>
                        {editId && actionWabaId && workspaceWabaId && actionWabaId !== workspaceWabaId && (
                          <p className="text-amber-700 dark:text-amber-400/90 font-medium">
                            Your header workspace does not match this action&apos;s number — chatbots are listed for the action&apos;s WABA. Switch workspace to that number to create a bot here, or use Reply materials after selecting the correct workspace.
                          </p>
                        )}
                      </div>
                    )}

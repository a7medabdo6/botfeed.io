...
const CataloguePage = () => {
  ...
  if (!selectedWabaId) {
    return <WabaRequired title="WABA Connection Required" description="To manage and link Facebook Business catalogues, you first need to connect a WhatsApp Business Account (WABA) to this workspace." />;
  }
            ...
            return (
              <Card key={catalog._id} className={`group relative overflow-hidden transition-all duration-300 border shadow-sm hover:shadow-lg rounded-lg ${isLinked ? "border-primary/20 bg-primary/2 dark:bg-primary/5" : "border-slate-100 dark:border-(--card-border-color) bg-white dark:bg-(--card-color) hover:border-primary/30"}`}>
                ...
                <CardFooter className="p-5 pt-0">
                  ...
                  {!isLinked && (
                    <Can permission="!update.ecommerce_catalogs">
                        <div className="w-full text-center text-xs text-gray-400 italic">No permission to link</div>
                    </Can>
                  )}
                  {isLinked && (
                     <Can permission="!update.ecommerce_catalogs">
                        <div className="w-full text-center text-xs text-primary font-bold">Linked</div>
                    </Can>
                  )}
                </CardFooter>

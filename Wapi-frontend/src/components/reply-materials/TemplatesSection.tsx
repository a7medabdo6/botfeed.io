...
const TemplatesSection: React.FC<TemplatesSectionProps> = ({ wabaId, onToggleSidebar }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 p-4 sm:p-6 overflow-y-auto custom-scrollbar">
      <CommonHeader title="Templates" description="Manage and design your WhatsApp message templates" featureKey="template_bots_used" onRefresh={handleRefresh} onSync={handleSync} onSyncStatus={handleSyncStatus} syncStatusPermission="create.template" isLoading={isLoading || isFetching} isSyncingStatus={isSyncingStatus} onAddClick={() => router.push(`/templates/${wabaId}/create`)} addLabel="Create New Template" addPermission="create.template" onSearch={setSearchQuery} searchTerm={searchQuery} searchPlaceholder="Search templates..." onToggleSidebar={onToggleSidebar} rightContent={WabaDisplay}>

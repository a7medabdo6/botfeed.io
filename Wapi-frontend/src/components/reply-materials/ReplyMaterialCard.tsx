      ...
      <div className={cn("relative h-36 flex items-center justify-center overflow-hidden", TYPE_ACCENT[item.type])}>
        {isImage && item.file_url ? (
          ...
          <div className="px-4 text-center flex flex-col items-center gap-2">
            <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2 leading-relaxed font-medium">{item.content || "No message body"}</p>
            <div className="px-3 py-1 bg-white dark:bg-(--dark-body) rounded-2xl border border-slate-200 dark:border-(--card-border-color) text-[10px] font-bold text-primary uppercase tracking-tight shadow-sm">{item.button_text || "Open Flow"}</div>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-2">

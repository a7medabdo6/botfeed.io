...
const CallAgentSchema = Yup.object().shape({
  ...
  ai_config: Yup.object().shape({
    ai_model: Yup.string().required("Please select an AI model"),
...
const CallAgentForm: React.FC<CallAgentFormProps> = ({ agent, onSave, isLoading }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      ai_config: {
        ai_model:
          typeof agent?.ai_config?.ai_model === "object" && (agent?.ai_config?.ai_model as { _id?: string })?._id
            ? String((agent.ai_config.ai_model as { _id: string })._id)
            : agent?.ai_config?.ai_model
              ? String(agent.ai_config.ai_model)
              : "",

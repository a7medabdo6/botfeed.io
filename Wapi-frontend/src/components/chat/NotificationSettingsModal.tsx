...
export const NotificationSettingsModal: React.FC<NotificationSettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl! max-w-[calc(100%-2rem)]! p-0! overflow-hidden border-none dark:bg-(--card-color) gap-1">
        <DialogHeader className="px-6 py-4 border-b border-slate-100 dark:border-(--card-border-color)">
        ...
        <div className="sm:p-6 p-4 max-h-102.5 overflow-auto custom-scrollbar pt-3 space-y-5">
          ...
          <div className="space-y-2">
            ...
            <Button variant="outline" onClick={() => setIsDND(!isDND)} className={cn("w-full h-12 justify-between px-4 rounded-lg border transition-all", isDND ? "bg-rose-50 border-rose-100 text-red-600 hover:text-red-700 hover:bg-rose-100 dark:bg-rose-500/10 dark:border-rose-500/20 dark:text-rose-400" : "bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100 dark:bg-(--page-body-bg) dark:border-(--card-border-color) dark:hover:bg-(--table-hover) dark:text-slate-300")}>
              ...
              <div className={cn("w-10 h-5 rounded-full relative transition-all", isDND ? "bg-red-500" : "bg-slate-300 dark:bg-slate-600")}>
                <div className={cn("absolute top-0.75 w-3 h-3 bg-white rounded-full transition-all", isDND ? "left-6" : "left-1")} />
              </div>
            </Button>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="h-96 overflow-y-scroll custom-scrollbar grid sm:grid-cols-2 grid-cols-1 gap-2 pr-3">
              {TONES.map((tone) => (
        ...
        <div className="p-4 sm:px-6 px-4 bg-slate-50 dark:bg-(--card-color) border-t border-slate-100 dark:border-(--card-border-color) flex justify-end gap-3">

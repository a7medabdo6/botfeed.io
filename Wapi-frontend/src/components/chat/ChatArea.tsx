...
const ChatArea: React.FC<ChatAreaProps> = ({ contactId, phoneNumberId, contactName, contactNumber, contactAvatar, isModal = false }) => {
  ...
  console.log("🚀 ~ ChatArea ~ isCustom:", isCustom);
  ...
  return (
    <div className={cn("relative flex-1 flex flex-col h-full dark:bg-(--card-color)! text-slate-900 dark:text-white bg-white border dark:border-(--card-border-color) rounded-t-none rounded-lg border-gray-100 overflow-hidden", !isModal && "max-w-[calc(100vw-364px)] [@media(max-width:991px)]:max-w-full")}>
      <div className="h-14 whitespace-nowrap flex-wrap [@media(min-width:768px)_and_(max-width:817px)]:h-30 [@media(max-width:430px)]:flex-wrap [@media(max-width:553px)]:h-35 [@media(max-width:430px)]:h-25 [@media(max-width:430px)]:gap-3 [@media(max-width:430px)]:p-2 [@media(max-width:452px)]:h-25 rounded-t-lg border-b border-gray-200 dark:bg-(--card-color)! dark:border-(--card-border-color) flex items-center justify-between sm:px-4 px-2 sticky top-0 z-10 cursor-pointer" style={{ backgroundColor: `${userSettingData?.theme_color == "null" ? "#FFFFFF" : "color-mix(in srgb, var(--chat-theme-color) , transparent 85%)"}` }}>
        {isMobileScreen && (
        ...
        <div className="flex items-center sm:gap-1 gap-0 [@media(max-width:430px)]:ml-auto rtl:[@media(max-width:430px)]:ml-0 rtl:[@media(max-width:430px)]:mr-auto [@media(max-width:430px)]:flex-wrap">
          ...
          {!isAgent && !isBaileys && (
            <Button variant="ghost" size="icon" onClick={() => router.push(`/campaigns/create?contact_id=${currentContactId}&redirect_to=${isModal ? "/contacts" : "/chat"}`)} className="dark:hover:bg-(--table-hover) dark:text-white transition-colors" style={isCustom ? { color: "var(--chat-theme-color)" } : {}} title="Send Template">
      ...
      <div className="flex-1 overflow-hidden flex flex-col relative">
        ...
        <div className="flex-1 relative z-10 flex flex-col min-h-0 pt-2">
          <ChatMessageList ref={scrollListRef} data={messagesData} isLoading={isLoading} onImageClick={(url) => handleImageClick(url, false)} isWindowExpired={isWindowExpired} />
        </div>
      </div>

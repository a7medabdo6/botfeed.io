...
const WeeklyMessagesChart = ({ data, isLoading }: WeeklyMessagesChartData) => {
  ...
  return (
    <div className="bg-white dark:bg-(--card-color) rounded-lg border border-slate-100 dark:border-(--card-border-color) sm:p-8 p-4 shadow-sm h-full">
      ...
      <div className="relative min-h-80">{isLoading ? <div className="absolute inset-0 bg-slate-50 dark:bg-(--card-color)! rounded-2xl animate-pulse" /> : data.length === 0 ? <div className="absolute inset-0 flex items-center justify-center text-slate-400 text-sm font-bold bg-slate-50 dark:bg-(--card-color)! rounded-lg border border-dashed dark:border-(--card-border-color) border-slate-100">Waiting for communication data...</div> : <ReactApexChart options={options} series={series} type="line" height={320} />}</div>
    </div>
  );
};

...
const Developer = () => {
  ...
  return (
    <div className="flex h-full bg-slate-50/50 dark:bg-(--dark-body) overflow-hidden relative">
      {sidebarOpen && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-60 lg:hidden" onClick={() => setSidebarOpen(false)} />}
      ...
      >
        <div className="relative h-full">
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden absolute top-3 right-3 z-50 w-8 h-8 rounded-lg bg-slate-100 dark:bg-(--card-color) flex items-center justify-center text-slate-500 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-(--table-hover) transition-colors shadow-sm" aria-label="Close sidebar">
            <X size={16} />
          </button>
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        ...
        <div className="flex-1 overflow-hidden">{activeTab === "dashboard" ? <ApiKeyManager /> : <ApiDocViewer sectionId={activeTab} />}</div>
      </div>
    </div>
  );
};

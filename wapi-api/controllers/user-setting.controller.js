...
export const updateUserSettings = async (req, res) => {
  try {
    const userId = req.user.id;
    const body = req.body || {};
    const { ai_model, api_key, is_show_phone_no, notification_tone, notifications_enabled, theme_color, user_bubble_color, contact_bubble_color, bg_color, user_text_color, contact_text_color } = body;
    ...
    if (userSettings) {
      ...
      if (req.files && req.files.bg_image) {
        ...
        userSettings.bg_color = null; // Clear bg_color if image is uploaded

...
import { getOAuth2Client, SCOPES, getCalendarClient, getSheetsClient } from '../utils/google-api-helper.js';
import { encrypt, decrypt } from '../utils/encryption-utils.js';
import { google } from 'googleapis';

/** Where users land after Google OAuth (your Next app), e.g. https://botfeed.io — no trailing slash. */
const getPublicFrontendOrigin = () => (process.env.FRONTEND_URL || 'http://localhost:3000').trim().replace(/\/+$/, '');
...
export const connect = async (req, res) => {
  try {
    ...
    const url = oauth2Client.generateAuthUrl({
      ...
      scope: SCOPES,
      prompt: 'consent',
      state: req.user.id // Pass user ID in state to verify on callback
    });
...
export const callback = async (req, res) => {
  try {
    ...
    const frontendUrl = getPublicFrontendOrigin();
...
export const linkCalendar = async (req, res) => {
  try {
    ...
    const cal = await GoogleCalendar.findByIdAndUpdate(id, { is_linked }, { new: true });
...
export const readSheet = async (req, res) => {
  try {
    ...
    const { range } = req.query; // e.g. 'Sheet1!A1:E10'

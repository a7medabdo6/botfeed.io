...
export const createCurrency = async (req, res) => {
  try {
    ...
    } else if (!setting.default_currency) {
...
export const getCurrencies = async (req, res) => {
  try {
    ...
    const currenciesWithDefault = currencies.map(curr => ({
      ...curr,
      is_default: curr._id.toString() === defaultCurrencyId
    }));
...
export const updateCurrency = async (req, res) => {
  try {
    ...
    if (updateData.is_active !== undefined) {
      const settings = await Setting.findOne().select('default_currency').lean();
      if (currency._id.toString() === settings?.default_currency.toString() && updateData.is_active === false) {
        return res.status(400).json({
          success: false,
          message: 'Default currency cannot be disabled. Please change the default currency first.'
        });
      }
...
export const deleteCurrencies = async (req, res) => {
  try {
    ...
    for (const currencyId of foundIds) {
      const isDefault = await Setting.findOne({ default_currency: currencyId });
      if (isDefault) {
...
export const toggleCurrencyStatus = async (req, res) => {
  try {
    ...
    if (currency.is_active) {
      const settings = await Setting.findOne().select('default_currency').lean();
      if (currency._id.toString() === settings?.default_currency.toString()) {
        return res.status(400).json({
          success: false,

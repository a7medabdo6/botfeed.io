  ...
  try {
    ...
    if (!contactId || !whatsappPhoneNumberId) {
      return res.status(400).json({
        success: false,
        error: 'Contact ID and WhatsApp Phone Number ID are required'
      });
    }
...
export const getMessages = async (req, res) => {
  try {
    ...
    if (replyIds.length > 0) {
      const replyMessages = await Message.find({ wa_message_id: { $in: replyIds } }).lean();
...
export const connectWhatsApp = async (req, res) => {
  try {
    const userId = req.user.id;
    const { name, provider = 'business_api', phone_number_id, access_token, whatsapp_business_account_id, registred_phone_number, app_id, workspace_id } = req.body;
...
export const getEmbbededSignupConnection = async (req, res) => {
  ...
  try {
    ...
    // Remove any existing WABA in this workspace that is different from the incoming one
    if (workspace_id) {
      const existingWaba = await WhatsappWaba.findOne({
        workspace_id: workspace_id,
          ...
          phoneNumbers.map(async (phone) => {
            ...
            try {
              const response = await axios.get(
                `https://graph.facebook.com/v19.0/${phone.phone_number_id}`,
                {
                  params: {
...
export const getBaileysQRCode = async (req, res) => {
  try {
    ...
    // If no QR code and connection is not active, trigger initialization
    // so a new QR is generated and pushed via socket event
    ...
    if (needsInit) {
      ...
      if (waba) {
        ...
        // Fire and forget — QR will arrive via socket 'whatsapp:connection:update'

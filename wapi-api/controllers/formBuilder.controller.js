...
export const createForm = async (req, res) => {
    try {
        let { waba_id, name, description, slug, category, is_active, is_multi_step, enable_recaptcha, submit_settings, appearance, contact_settings, fields } = req.body;
        const userId = req.user.id;
        ...
        const form = await Form.create({
            ...
            is_multi_step: is_multi_step ?? false,
    ...
    const screens = sortedSteps.map((step, index) => {
        ...
        return {
            ...
            data: incomingFields.reduce((acc, name) => {
                acc[name] = { type: "string", __example__: "test" };
                return acc;
...
const getMetaFlowJson = async (flowId, accessToken) => {
    try {
        ...
        console.error(`Error fetching flow JSON for ${flowId}:`, err.response?.data || err.message);
        return null;
    }
};
...
export const syncMetaFlow = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("Meta API Fetch Error:", metaError.response?.data || metaError.message);
            return res.status(502).json({
                success: false,
                error: "Failed to fetch flows from Meta API",
        ...
        return res.status(500).json({
            success: false,
            error: "Failed to sync meta flows",
...
export const retriveMetaFlow = async (flow_id, accessToken) => {
    try {
        ...
        console.error("Retrieve Meta Flow Error:", err?.response?.data || err.message);
        return null;
    }
};
...
export const deleteMetaFlow = async (flow_id, accessToken) => {
    try {
        ...
        console.error("Delete Meta Flow Error:", err?.response?.data || err.message);
        return null;
    }
};
...
export const deprecateMetaFlow = async (flow_id, accessToken) => {
    try {
        ...
        console.error("Deprecate Meta Flow Error:", err?.response?.data || err.message);
        return null;
    }
};
...
export const updateForm = async (req, res) => {
    try {
        ...
        if (updateData.is_multi_step !== undefined) form.is_multi_step = updateData.is_multi_step;
        if (updateData.enable_recaptcha !== undefined) form.enable_recaptcha = updateData.enable_recaptcha;
        if (updateData.submit_settings) form.submit_settings = { ...form.submit_settings, ...updateData.submit_settings };
        if (updateData.appearance) form.appearance = { ...form.appearance, ...updateData.appearance };
        if (updateData.contact_settings) form.contact_settings = { ...form.contact_settings, ...updateData.contact_settings };

...
export const createMessageBot = async (req, res) => {
    try {
        ...
        const validReplyTypes = ['text', 'media', 'template', 'catalog', 'chatbot', 'agent', 'sequence', 'flow'];
...
export const getAllMessageBots = async (req, res) => {
    try {
        const userId = req.user?.owner_id;
        const { waba_id, page = 1, limit = 10, search = '', status } = req.query;
        ...
        if (search) {
            query.keywords = { $elemMatch: { $regex: search, $options: 'i' } };
        }

        const skip = (parseInt(page) - 1) * parseInt(limit);
        const bots = await MessageBot.find(query)
            .select('_id keywords matching_method reply_type reply_id status')
            .sort({ createdAt: -1 })
            .skip(skip)

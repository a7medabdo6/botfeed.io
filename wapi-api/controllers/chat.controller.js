...
export const getChatLabels = async (req, res) => {
  try {
    ...
    const userId = req.user.owner_id; // Using owner context for connection lookup
...
export const assignChatToAgent = async (req, res) => {
  try {
    ...
    if (existingAssignment) {
      ...
      existingAssignment.assigned_by = req.user.id; // Corrected to use current user as assigner

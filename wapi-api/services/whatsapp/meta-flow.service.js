...
class MetaFlowService {
    async handleFlowSubmission(message, whatsappPhoneNumber, contactDoc) {
        try {
            ...
            if (form.contact_settings?.auto_create_contact) {
                const nameValue = mappedFields.find(f => /name/i.test(f.label || ''))?.value;
                const emailValue = mappedFields.find(f => /email/i.test(f.label || ''))?.value;

                if (!contactDoc) {
                    contactDoc = await Contact.create({
                        phone_number: message.from,
                        name: nameValue || message.from,
                        email: emailValue || null,
                        source: 'whatsapp',
                        created_by: form.user_id,
                        status: 'lead'
                    });
                    console.log(`[MetaFlowService] Contact ${contactDoc._id} created from submission`);
                } else {
                    const updates = {};
                    if (nameValue && (contactDoc.name === contactDoc.phone_number || !contactDoc.name)) updates.name = nameValue;
                    if (emailValue && !contactDoc.email) updates.email = emailValue;

                    if (Object.keys(updates).length > 0) {
                        await Contact.findByIdAndUpdate(contactDoc._id, { $set: updates });
                        console.log(`[MetaFlowService] Contact ${contactDoc._id} enriched from submission`);
                    }
                }
            }

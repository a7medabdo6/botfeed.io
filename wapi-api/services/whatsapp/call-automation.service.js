import { WhatsappCallAgent, WhatsappCallLog, WhatsappCallSetting, Contact, WhatsappPhoneNumber } from '../../models/index.js';
...
const callAgentAiPopulate = {
    path: 'ai_config.ai_model',
    match: { deleted_at: null },
    select: 'display_name provider model_id api_endpoint api_version config headers_template request_format response_path status'
};

const findCallAgentById = (id) =>
    WhatsappCallAgent.findById(id).populate(callAgentAiPopulate);
...
class CallAutomationService {
    async handleCallWebhook(callData, phoneNumberId) {
        ...
        try {
            ...
            switch (event) {
                ...
                case 'connected':
                    if (callLog) {
                        const agent = await findCallAgentById(callLog.agent_id);
                        if (agent) {
    ...
    async resolveContactAndAgent(contactNumber, phoneNumberId) {
        ...
        if (contact && contact.assigned_call_agent_id) {
            agent = await findCallAgentById(contact.assigned_call_agent_id);
        }

        if (!agent) {
            const settings = await WhatsappCallSetting.findOne({ phone_number_id: phoneNumberId });
            if (settings && settings.fallback_agent_id) {
                agent = await findCallAgentById(settings.fallback_agent_id);
            }
        }
...
async executeFunctionWithCollectedParams(functionDef, params, callLog) {
    ...
    async processAIRequest(agent, params, callLog) {
        const fullAgent = agent?._id
            ? await findCallAgentById(agent._id)
            : agent;
        if (!fullAgent) {
            throw new Error('Call agent not found');
        }

        const { prompt, training_url, include_concise_instruction, api_key: agentApiKey } = fullAgent.ai_config;
        const aiModelDoc = fullAgent.ai_config?.ai_model;
        ...
        const modelPayload = aiModelDoc && typeof aiModelDoc === 'object' && aiModelDoc.provider
            ? aiModelDoc.toObject ? aiModelDoc.toObject() : aiModelDoc
            : null;

        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                if (modelPayload && agentApiKey) {
                    ...
                    const cleanedText = String(aiText || '').replace(/\n+/g, ' ').substring(0, 300);
                    console.log(`[AI] Cleaned response (${modelPayload.provider}): ${cleanedText}`);
                    return cleanedText;
                }

                const legacyModelId = fullAgent.ai_config?.model_id;
                if (!legacyModelId || !agentApiKey) {
                    throw new Error('Configure an AI model (or legacy model id) and API key for this call agent.');
                }

                const response = await axios.post(
                    `https://generativelanguage.googleapis.com/v1/models/${legacyModelId}:generateContent?key=${agentApiKey}`,
                    {
                        contents: [
                            {
                                parts: [
                ...
                console.log(`[AI] Cleaned response (legacy Gemini): ${cleanedText}`);
                return cleanedText;
            } catch (error) {
                lastError = error;
                ...
                if (is429 && attempt < MAX_RETRIES) {
                    ...
                    console.warn(`[AI] Rate limit (attempt ${attempt}/${MAX_RETRIES}). Retrying in ${delayMs}ms...`);
                    await new Promise(resolve => setTimeout(resolve, delayMs));
                } else {
                    console.error(`[AI] Request failed after ${attempt} attempt(s):`, error.response?.data || error.message);
                    throw error;
                }
            }
        }
    ...
    async initiateOutboundCall(phoneNumberId, contactNumber, agentId, userId, campaignId = null) {
        try {
            ...
            const agent = await findCallAgentById(agentId);
    ...
    async handleOutboundCallConnected(waCallId, session, callbackData, phoneNumberId) {
        try {
            ...
            const agent = await findCallAgentById(agent_id);
            if (!agent) {

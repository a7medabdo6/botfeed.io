...
export default class BusinessAPIProvider extends BaseProvider {
  ...
  buildWhatsAppPayload(params) {
    ...
    switch (messageType) {
      ...
      case MESSAGE_TYPES.INTERACTIVE:
        const { interactiveType, buttonParams, listParams, ctaUrlParams } = params;
        ...
        } else if (interactiveType === 'cta_url' && ctaUrlParams) {
          payload.interactive = {
            type: 'cta_url',
            body: {
              text: messageText || ' '
            ...
            action: {
              name: 'cta_url',
              parameters: {
                display_text: ctaUrlParams.display_text || 'Open',
                url: ctaUrlParams.url || ''
              }
            }
          };
        }
  ...
  async sendMessage(userId, params, connection = null) {
    ...
    if (messageType === 'interactive') {
      ...
      if (interactiveType === 'cta_url' && ctaUrlParams) {
        whatsappPayload = {
          messaging_product: 'whatsapp',
          to: recipientNumber,
          type: 'interactive',

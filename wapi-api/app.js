...
// Debug route - before all middleware
  ...
  cors({
    origin: function (origin, callback) {
      ...
      if (allowedOrigins.length === 0) {
        return callback(new Error("CORS blocked (ALLOWED_ORIGINS empty): " + origin));
      }
...
const captureRawBody = (req, res, next) => {
  if (req.originalUrl === "/api/webhook/stripe" || req.originalUrl === "/api/webhook/razorpay" || req.originalUrl === "/api/webhook/paypal") {
    let data = "";
...
import unifiedChatRoutes from "./routes/unified-chat.routes.js";
